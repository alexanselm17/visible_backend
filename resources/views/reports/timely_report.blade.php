    ...
    <table>
        <thead>
            <tr>
                ...
                <th>Product</th>
                <th>In Charge</th>
                <th>Start Vol (Ltrs)</th>
                <th>End Vol (Ltrs)</th>
                <th>Start Cash (Ltrs)</th>
                <th>End Cash (Ltrs)</th>
                <th>Qty Sold</th>
        ...
        <tbody>
            @foreach($pumps as $pump)
                ...
                <tr>
                    ...
                    <td>{{ $pump->products  }}</td>
                    <td>{{ $pump->in_charge->fullname  }}</td>
                    <td>{{ $pump->start_volume  }}</td>
                    <td>{{ $pump->ended_volume  }}</td>
                 <td>{{ isset($pump->start_cash) ? number_format($pump->start_cash, 2) : '0.00' }}</td>
<td>{{ isset($pump->ended_cash) ? number_format($pump->ended_cash, 2) : '0.00' }}</td>

                   <td>
    {{ $pump && $pump->ended_volume !== null 
        ? number_format(($pump->ended_volume ?? 0) - ($pump->start_volume ?? 0), 2) 
        : '0.00' }}
</td>

<td>
    {{ $pump && $pump->ended_cash !== null 
        ? number_format(($pump->ended_cash ?? 0) - ($pump->start_cash ?? 0), 2) 
        : '0.00' }}
</td>


<td>
    {{ number_format((($pump->ended_volume ?? 0) - ($pump->start_volume ?? 0)) * ($pump->price ?? 0), 2) }}
</td>
...
<!-- 
    ...
    <div class="content">
        <h2>{{ $pump->my_pump->name ?? 'N/A' }} - {{ $pump->shift->description ?? 'N/A' }}</h2>
        ...
        <table>
            ...
            <tbody>
                <tr>
                    <td>{{ $pump->products ?? 'N/A' }}</td>
                    <td>{{ $pump->start_volume }}</td>
                    <td>{{$pump->ended_volume }}</td>
                    <td>{{ is_numeric($pump->start_cash) ? number_format($pump->start_cash, 2) : $pump->start_cash }}</td>
                    <td>{{ is_numeric($pump->ended_cash) ? number_format($pump->ended_cash, 2) : $pump->ended_cash }}</td>
                </tr>
            </tbody>
        </table>
        ...
        <div class="overal-report">
            <h3>{{ $pump->my_pump->name ?? 'N/A'}}</h3>
            <p><b>Incharge:</b> {{ $pump->in_charge->fullname }}</p>
            <p><b>Total:</b> {{ number_format($pump->ended_volume - $pump->start_volume , 2) }} Ltrs</p>
            <p><b>Total: Ksh.</b> {{ number_format( $pump->ended_cash -  $pump->start_cash, 2) }}</p>
            <p><b>Calculated: Ksh.</b> {{ number_format(($pump->ended_volume - $pump->start_volume)*$pump->price , 2) }}</p>
        </div>
    </div>
    @endforeach
