...
<body>
    ...
    <div class="overal-report">
      ...
      <div class="content">
        ...
        <div class="pagination">
            <ul class="pagination">
                <li>
                    <a href="{{ $sales->previousPageUrl() }}" class="prev" {{ $invoiceRepayments->onFirstPage() ? 'style=pointer-events:none;color:gray;' : '' }}>
                        Previous
                    </a>
                </li>
                <li>
                    <a href="{{ $sales->nextPageUrl() }}" class="next" {{ !$invoiceRepayments->hasMorePages() ? 'style=pointer-events:none;color:gray;' : '' }}>
                        Next
                    </a>
                </li>
            </ul>
    ...
    <div class="content">
        ...
        <div class="pagination">
            <ul class="pagination">
                <li>
                    <a href="{{ $sales->previousPageUrl() }}" class="prev" {{ $bankings->onFirstPage() ? 'style=pointer-events:none;color:gray;' : '' }}>
                        Previous
                    </a>
                </li>
                <li>
                    <a href="{{ $sales->nextPageUrl() }}" class="next" {{ !$bankings->hasMorePages() ? 'style=pointer-events:none;color:gray;' : '' }}>
                        Next
                    </a>
                </li>
            </ul>
                    ...
                    <div class="shift-details" style="position: relative; padding-bottom: 80px;">
                        @if(count($shift['products']) > 0)
                            <table>
                                ...
                                <tbody>
                                @foreach($shift['products'] as $index => $product)
    <tr>
        <td>{{ $index + 1 }}</td>
        <td>{{ $product['product_name'] ?? 'N/A' }}</td>
        <td>{{ $product['opening_stock'] ?? 'N/A' }}</td>
        <td>{{ $product['closing_stock'] ?? 'N/A' }}</td>
...
@endforeach
                        ...
                        @else
                            <p>No product data available for this shift.</p>
                        @endif

                        <!-- Shift assignment details positioned at the bottom right with extra margin for spacing -->
                        <div class="shift-assignment-details" style="position: absolute; bottom: 0; right: 0; text-align: right; margin-top: 20px;"> <!-- Added margin-top for spacing -->
                            <p>Assigned By: {{ $shift['assigned_by'] ?? 'N/A' }}</p>
                            <p>Assigned To: {{ $shift['assigned_to'] ?? 'N/A' }}</p>
                        </div>
                    </div>
                @endforeach
            @else
                <p>No shifts available for this station.</p>
            @endif
        </div>
    @endforeach
